...
static int get_feature_report(struct os_hid_device *handle, rift_sensor_feature_cmd cmd, unsigned char* buf)
{
	memset(buf, 0, FEATURE_BUFFER_SIZE);
	return os_hid_get_feature(handle, (uint8_t)cmd, buf, FEATURE_BUFFER_SIZE);
}

static int send_feature_report(struct os_hid_device *handle, unsigned char* buf, int length)
{
	return os_hid_set_feature(handle, buf, (size_t)length);
}

static int rift_hmd_radio_get_cmd_response(struct os_hid_device *handle, bool wait)
{
...
static int rift_hmd_radio_send_cmd(struct os_hid_device *handle, uint8_t a, uint8_t b, uint8_t c)
{
	unsigned char buffer[FEATURE_BUFFER_SIZE];
...
void rift_hmd_radio_init(rift_hmd_radio_state *radio, struct os_hid_device *handle)
{
	radio->handle = handle;
}
...
bool rift_hmd_radio_get_address(struct os_hid_device *handle, uint8_t radio_address[5])
{
